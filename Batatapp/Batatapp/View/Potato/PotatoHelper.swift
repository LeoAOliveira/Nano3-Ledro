...
public class PotatoHelper {
    ...
    public static func throwPotato(_ potato: ModelEntity, _ to: Entity?) {
        if potato.physicsBody == nil {
            let motion = PhysicsMotionComponent()
            let collision = CollisionComponent(shapes: [.generateSphere(radius: 5)])
        ...
        potato.applyImpulse([4000, 0, -3000], at: [0,0,0], relativeTo: to)
    }
}
